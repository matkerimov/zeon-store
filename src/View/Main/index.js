import React, {useEffect, useState} from 'react';
import './style.css'
import sale_img from '../Main/img/123.png'
import card_img from '../Main/img/Rectangle 491.png'
import collection_img from '../Main/img/collection.png'
import {Breadcrumb, Button, Card, CardGroup, Carousel} from 'react-bootstrap';
import {Link} from "react-router-dom";
import axios from "axios";
import Swiper from 'swiper';


const Main = () => {
    const [products, setProducts] = useState([])
    const bestSellers = products.filter(item => item.isHit = true)
    const newProducts = products.filter(item => item.isNew = true)
    const [moreProduct, setMoreProduct] = useState(false)
    const [moreNovelty, setMoreNovelty] = useState(false)

    const [collection, setCollection] = useState([])
    const [moreCollection, setMoreCollection] = useState(false)



    useEffect(() => {
        axios(`https://613cea45270b96001798b2e8.mockapi.io/api/product`)
            .then(({data}) => {
                setProducts(data)
            })
    }, [])

    useEffect(() => {
        axios(`https://613cea45270b96001798b2e8.mockapi.io/api/collection`)
            .then(({data}) => {
                setCollection(data)
            })
    })

    return (
        <div className=" container ">

            <Carousel>
                <Carousel.Item>
                    <img className="main__carousel__img" src={sale_img} alt="sale_img"/>

                </Carousel.Item>
                <Carousel.Item>
                    <img className="main__carousel__img" src={sale_img} alt="sale_img"/>


                </Carousel.Item>
                <Carousel.Item>
                    <img className="main__carousel__img" src={sale_img} alt="sale_img"/>


                </Carousel.Item>
            </Carousel>



            <section className="main">
                <h4 className="main__title">Хит продаж</h4>


                <div className="media">
                    {
                        bestSellers.slice(0, moreProduct ? 16 : 8).map(item => {
                            return (
                                <div  key={item.id}>
                                    <Link to="/about_dress">
                                        <Card item={item} className="main__card">
                                            <Carousel fade>
                                            {
                                                item?.images?.map(el =>{
                                                    return (

                                                            <Carousel.Item>
                                                                <Card.Img variant="top" src={el}/>

                                                            </Carousel.Item>


                                                        )


                                                })
                                            }
                                            </Carousel>

                                            <Card.Body>
                                                <p className="card_title">{item.title}</p>
                                                <p className="card_price">
                                                    {item.price} p
                                                </p>
                                                <p className="card_info">
                                                    Размер: {item.size}
                                                </p>
                                                <div>
                                                    <small className="text-muted">
                                                        <svg width="162" height="16" viewBox="0 0 162 16" fill="none"
                                                             xmlns="http://www.w3.org/2000/svg">
                                                            <rect opacity="0.47" y="4" width="8" height="8" rx="4"
                                                                  fill="#73A39D"/>
                                                            <rect opacity="0.47" x="20.8574" y="4" width="8" height="8"
                                                                  rx="4"
                                                                  fill="#84CC4C"/>
                                                            <rect x="45.7148" y="4" width="8" height="8" rx="4"
                                                                  fill="#B5A8A1"/>
                                                            <rect x="70.5723" y="4" width="8" height="8" rx="4"
                                                                  fill="#AB844A"/>
                                                            <rect opacity="0.47" x="91.4277" y="4" width="8" height="8"
                                                                  rx="4"
                                                                  fill="#6977F0"/>
                                                            <rect opacity="0.47" x="111.785" y="3.5" width="9"
                                                                  height="9" rx="4.5"
                                                                  fill="white"
                                                                  stroke="#D1D1D1"/>
                                                            <rect opacity="0.47" x="133.143" y="4" width="8" height="8"
                                                                  rx="4"
                                                                  fill="#141414"/>
                                                            <rect opacity="0.47" x="154" y="4" width="8" height="8"
                                                                  rx="4" fill="#FF0000"/>
                                                        </svg>

                                                    </small>
                                                </div>
                                            </Card.Body>

                                        </Card>

                                    </Link>

                                </div>
                            )
                        })
                    }
                </div>


                <button onClick={ () => setMoreProduct(true)}  className="more_btn ">Еще</button>
            </section>
            <section className="novelty main ">
                <h4 className="main__title">Новинки</h4>
                <div className="media">
                    {
                        newProducts.slice(0, moreNovelty ? 12 : 4).map(item => {
                            return (
                                <div key={item.id}>
                                    <Link to="/about_dress">
                                        <Card item={item} className="main__card">

                                            <Carousel fade>
                                                {
                                                    item?.images?.map(el =>{
                                                        return (

                                                            <Carousel.Item>
                                                                <Card.Img variant="top" src={el}/>

                                                            </Carousel.Item>


                                                        )


                                                    })
                                                }
                                            </Carousel>

                                            <Card.Body>
                                                <p className="card_title">{item.title}</p>
                                                <p className="card_price">
                                                    {item.price} p
                                                </p>
                                                <p className="card_info">
                                                    Размер: {item.size}
                                                </p>
                                                <div>
                                                    <small className="text-muted">
                                                        <svg width="162" height="16" viewBox="0 0 162 16" fill="none"
                                                             xmlns="http://www.w3.org/2000/svg">
                                                            <rect opacity="0.47" y="4" width="8" height="8" rx="4"
                                                                  fill="#73A39D"/>
                                                            <rect opacity="0.47" x="20.8574" y="4" width="8" height="8"
                                                                  rx="4"
                                                                  fill="#84CC4C"/>
                                                            <rect x="45.7148" y="4" width="8" height="8" rx="4"
                                                                  fill="#B5A8A1"/>
                                                            <rect x="70.5723" y="4" width="8" height="8" rx="4"
                                                                  fill="#AB844A"/>
                                                            <rect opacity="0.47" x="91.4277" y="4" width="8" height="8"
                                                                  rx="4"
                                                                  fill="#6977F0"/>
                                                            <rect opacity="0.47" x="111.785" y="3.5" width="9"
                                                                  height="9" rx="4.5"
                                                                  fill="white"
                                                                  stroke="#D1D1D1"/>
                                                            <rect opacity="0.47" x="133.143" y="4" width="8" height="8"
                                                                  rx="4"
                                                                  fill="#141414"/>
                                                            <rect opacity="0.47" x="154" y="4" width="8" height="8"
                                                                  rx="4" fill="#FF0000"/>
                                                        </svg>

                                                    </small>
                                                </div>
                                            </Card.Body>

                                        </Card>
                                    </Link>
                                </div>
                            )
                        })
                    }


                </div>
                <button onClick={() => setMoreNovelty(true)}  className="more_btn ">Еще</button>
            </section>
            <section className="collection main">
                <h4 className="main__title">Коллекция</h4>
                <div className="media">
                    {
                        collection.slice(0, moreCollection ? 8 : 4 ).map(item => {
                            return (
                                <div key={item.id}>
                                    <Link to="/collections">
                                        <Card className="main__card collection__card">
                                            <Card.Img variant="top" src={item.image}/>
                                            {/*<Card.Body>*/}
                                            <Card.Text className="collection__title">
                                                {item.title}
                                            </Card.Text>

                                            {/*</Card.Body>*/}
                                            <Card.Footer className="collection__footer">
                                                    <Card.Text className="collection__text">
                                                        Смотреть все
                                                        <span>
                                                             <i className="fa-solid fa-chevron-right fa-lg" />
                                                        </span>
                                                    </Card.Text>

                                            </Card.Footer>
                                        </Card>
                                    </Link>
                                </div>
                            )
                        })
                    }
                </div>

                <button onClick={() => setMoreCollection(true)}  className="more_btn ">Еще</button>
            </section>
            <section className="advantages main">
                <h4 className="main__title">Наши преимущества</h4>
                <div className="media">
                    <Card className="main__card advantages__card py-3">
                        <Card.Body style={{padding: "none"}}>
                            <span className="advantages__svg ">
                                <svg width="70" height="70" viewBox="0 0 70 70" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
<path
    d="M35.6473 30.9187C35.5178 30.8907 35.3849 30.8856 35.2537 30.9052C33.939 30.7522 32.9472 29.6399 32.945 28.3167C32.945 27.6827 32.4308 27.1685 31.7968 27.1685C31.1627 27.1685 30.6486 27.6827 30.6486 28.3167C30.6519 30.5262 32.1298 32.4616 34.2602 33.0463V34.1946C34.2602 34.8286 34.7743 35.3428 35.4084 35.3428C36.0425 35.3428 36.5566 34.8286 36.5566 34.1946V33.1159C39.0156 32.6012 40.6869 30.311 40.4285 27.8122C40.1694 25.3134 38.0642 23.4145 35.552 23.4139C34.1122 23.4139 32.945 22.2466 32.945 20.8069C32.945 19.3672 34.1122 18.1999 35.552 18.1999C36.9917 18.1999 38.159 19.3672 38.159 20.8069C38.159 21.441 38.6731 21.9551 39.3072 21.9551C39.9413 21.9551 40.4554 21.441 40.4554 20.8069C40.452 18.4869 38.8267 16.486 36.5566 16.0072V14.9622C36.5566 14.3281 36.0425 13.814 35.4084 13.814C34.7743 13.814 34.2602 14.3281 34.2602 14.9622V16.0773C31.8909 16.7243 30.366 19.0246 30.6923 21.4589C31.0186 23.8933 33.0958 25.7103 35.552 25.7097C36.9726 25.7097 38.1315 26.8473 38.1573 28.2674C38.1836 29.6875 37.0668 30.8671 35.6473 30.9187Z"
    fill="#1D1D1B"/>
<path
    d="M18.9023 24.6501C18.9023 33.8458 26.3567 41.3001 35.5524 41.3001C44.7481 41.3001 52.2024 33.8458 52.2024 24.6501C52.2024 15.4543 44.7481 8 35.5524 8C26.3612 8.01009 18.913 15.4588 18.9023 24.6501ZM35.5524 10.2964C43.4799 10.2964 49.906 16.7225 49.906 24.6501C49.906 32.5776 43.4799 39.0037 35.5524 39.0037C27.6249 39.0037 21.1987 32.5776 21.1987 24.6501C21.2077 16.7264 27.6288 10.3054 35.5524 10.2964Z"
    fill="#1D1D1B"/>
<path
    d="M57.8633 44.1309L48.8689 48.3806C47.6271 46.0707 45.2449 44.6007 42.6233 44.5267L34.3202 44.2985C33.0054 44.261 31.7154 43.9279 30.5464 43.3241L29.7016 42.8863C25.3756 40.6314 20.2194 40.637 15.8979 42.9008L15.9506 40.9868C15.968 40.3527 15.4684 39.8246 14.8344 39.8072L5.72947 39.5566C5.09538 39.5392 4.56781 40.0387 4.55043 40.6728L4.00044 60.6531C3.98306 61.2872 4.4826 61.8153 5.11669 61.8327L14.2216 62.0833H14.253C14.8753 62.0833 15.3843 61.5883 15.4012 60.9665L15.4275 60.0106L17.7929 58.7441C18.7213 58.2446 19.8067 58.1257 20.8209 58.4128L34.9458 62.3771C34.9705 62.3844 34.9952 62.39 35.0204 62.3956C36.0413 62.6092 37.0819 62.7163 38.1247 62.7157C40.3336 62.7169 42.5168 62.2409 44.5245 61.3192C44.5738 61.2968 44.6214 61.2704 44.6669 61.2407L65.1467 47.9937C65.6569 47.6635 65.8217 46.9936 65.5229 46.4643C64.0024 43.7665 60.6301 42.7388 57.8633 44.1309ZM6.32768 59.5688L6.81432 41.8838L13.6234 42.0711L13.1367 59.7567L6.32768 59.5688ZM43.4901 59.2683C40.9958 60.3941 38.2099 60.7047 35.5289 60.157L21.4421 56.2027C19.8566 55.7548 18.1595 55.9403 16.7092 56.7202L15.5004 57.3677L15.8239 45.6093C19.6542 43.0617 24.5666 42.7999 28.6459 44.9253L29.4907 45.3632C30.9675 46.1257 32.5962 46.5462 34.2574 46.5944L42.5611 46.8226C44.8934 46.8898 46.8741 48.5505 47.3473 50.8357L34.9789 50.4942C34.3448 50.4768 33.8173 50.9769 33.7993 51.6105C33.7819 52.2446 34.282 52.7727 34.9156 52.7901L48.565 53.1657H48.597C49.2187 53.1651 49.7272 52.6701 49.7441 52.0489C49.7581 51.5499 49.7211 51.0504 49.6331 50.5593L58.8585 46.2002C58.8675 46.1963 58.8764 46.1918 58.8854 46.1873C60.1939 45.5252 61.7761 45.7467 62.8525 46.7429L43.4901 59.2683Z"
    fill="#1D1D1B"/>
</svg>

                            </span>
                            <Card.Title className="advantages__title mt-4">Удобные способы оплаты</Card.Title>
                            <Card.Subtitle className="advantages__subtitle text-muted mt-2">Мы предлагаем
                                возможность безналичной оплаты</Card.Subtitle>

                        </Card.Body>
                    </Card>
                    <Card className="main__card advantages__card py-3">
                        <Card.Body style={{padding: "none"}}>
                            <span className="advantages__svg ">
                                <svg width="70" height="70" viewBox="0 0 70 70" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
<path
    d="M35.6473 30.9187C35.5178 30.8907 35.3849 30.8856 35.2537 30.9052C33.939 30.7522 32.9472 29.6399 32.945 28.3167C32.945 27.6827 32.4308 27.1685 31.7968 27.1685C31.1627 27.1685 30.6486 27.6827 30.6486 28.3167C30.6519 30.5262 32.1298 32.4616 34.2602 33.0463V34.1946C34.2602 34.8286 34.7743 35.3428 35.4084 35.3428C36.0425 35.3428 36.5566 34.8286 36.5566 34.1946V33.1159C39.0156 32.6012 40.6869 30.311 40.4285 27.8122C40.1694 25.3134 38.0642 23.4145 35.552 23.4139C34.1122 23.4139 32.945 22.2466 32.945 20.8069C32.945 19.3672 34.1122 18.1999 35.552 18.1999C36.9917 18.1999 38.159 19.3672 38.159 20.8069C38.159 21.441 38.6731 21.9551 39.3072 21.9551C39.9413 21.9551 40.4554 21.441 40.4554 20.8069C40.452 18.4869 38.8267 16.486 36.5566 16.0072V14.9622C36.5566 14.3281 36.0425 13.814 35.4084 13.814C34.7743 13.814 34.2602 14.3281 34.2602 14.9622V16.0773C31.8909 16.7243 30.366 19.0246 30.6923 21.4589C31.0186 23.8933 33.0958 25.7103 35.552 25.7097C36.9726 25.7097 38.1315 26.8473 38.1573 28.2674C38.1836 29.6875 37.0668 30.8671 35.6473 30.9187Z"
    fill="#1D1D1B"/>
<path
    d="M18.9023 24.6501C18.9023 33.8458 26.3567 41.3001 35.5524 41.3001C44.7481 41.3001 52.2024 33.8458 52.2024 24.6501C52.2024 15.4543 44.7481 8 35.5524 8C26.3612 8.01009 18.913 15.4588 18.9023 24.6501ZM35.5524 10.2964C43.4799 10.2964 49.906 16.7225 49.906 24.6501C49.906 32.5776 43.4799 39.0037 35.5524 39.0037C27.6249 39.0037 21.1987 32.5776 21.1987 24.6501C21.2077 16.7264 27.6288 10.3054 35.5524 10.2964Z"
    fill="#1D1D1B"/>
<path
    d="M57.8633 44.1309L48.8689 48.3806C47.6271 46.0707 45.2449 44.6007 42.6233 44.5267L34.3202 44.2985C33.0054 44.261 31.7154 43.9279 30.5464 43.3241L29.7016 42.8863C25.3756 40.6314 20.2194 40.637 15.8979 42.9008L15.9506 40.9868C15.968 40.3527 15.4684 39.8246 14.8344 39.8072L5.72947 39.5566C5.09538 39.5392 4.56781 40.0387 4.55043 40.6728L4.00044 60.6531C3.98306 61.2872 4.4826 61.8153 5.11669 61.8327L14.2216 62.0833H14.253C14.8753 62.0833 15.3843 61.5883 15.4012 60.9665L15.4275 60.0106L17.7929 58.7441C18.7213 58.2446 19.8067 58.1257 20.8209 58.4128L34.9458 62.3771C34.9705 62.3844 34.9952 62.39 35.0204 62.3956C36.0413 62.6092 37.0819 62.7163 38.1247 62.7157C40.3336 62.7169 42.5168 62.2409 44.5245 61.3192C44.5738 61.2968 44.6214 61.2704 44.6669 61.2407L65.1467 47.9937C65.6569 47.6635 65.8217 46.9936 65.5229 46.4643C64.0024 43.7665 60.6301 42.7388 57.8633 44.1309ZM6.32768 59.5688L6.81432 41.8838L13.6234 42.0711L13.1367 59.7567L6.32768 59.5688ZM43.4901 59.2683C40.9958 60.3941 38.2099 60.7047 35.5289 60.157L21.4421 56.2027C19.8566 55.7548 18.1595 55.9403 16.7092 56.7202L15.5004 57.3677L15.8239 45.6093C19.6542 43.0617 24.5666 42.7999 28.6459 44.9253L29.4907 45.3632C30.9675 46.1257 32.5962 46.5462 34.2574 46.5944L42.5611 46.8226C44.8934 46.8898 46.8741 48.5505 47.3473 50.8357L34.9789 50.4942C34.3448 50.4768 33.8173 50.9769 33.7993 51.6105C33.7819 52.2446 34.282 52.7727 34.9156 52.7901L48.565 53.1657H48.597C49.2187 53.1651 49.7272 52.6701 49.7441 52.0489C49.7581 51.5499 49.7211 51.0504 49.6331 50.5593L58.8585 46.2002C58.8675 46.1963 58.8764 46.1918 58.8854 46.1873C60.1939 45.5252 61.7761 45.7467 62.8525 46.7429L43.4901 59.2683Z"
    fill="#1D1D1B"/>
</svg>

                            </span>
                            <Card.Title className="advantages__title mt-4">Удобные способы оплаты</Card.Title>
                            <Card.Subtitle className="advantages__subtitle text-muted mt-2">Мы предлагаем
                                возможность безналичной оплаты</Card.Subtitle>

                        </Card.Body>
                    </Card>
                    <Card className="main__card advantages__card py-3">
                        <Card.Body style={{padding: "none"}}>
                            <span className="advantages__svg ">
                                <svg width="70" height="70" viewBox="0 0 70 70" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
<path
    d="M35.6473 30.9187C35.5178 30.8907 35.3849 30.8856 35.2537 30.9052C33.939 30.7522 32.9472 29.6399 32.945 28.3167C32.945 27.6827 32.4308 27.1685 31.7968 27.1685C31.1627 27.1685 30.6486 27.6827 30.6486 28.3167C30.6519 30.5262 32.1298 32.4616 34.2602 33.0463V34.1946C34.2602 34.8286 34.7743 35.3428 35.4084 35.3428C36.0425 35.3428 36.5566 34.8286 36.5566 34.1946V33.1159C39.0156 32.6012 40.6869 30.311 40.4285 27.8122C40.1694 25.3134 38.0642 23.4145 35.552 23.4139C34.1122 23.4139 32.945 22.2466 32.945 20.8069C32.945 19.3672 34.1122 18.1999 35.552 18.1999C36.9917 18.1999 38.159 19.3672 38.159 20.8069C38.159 21.441 38.6731 21.9551 39.3072 21.9551C39.9413 21.9551 40.4554 21.441 40.4554 20.8069C40.452 18.4869 38.8267 16.486 36.5566 16.0072V14.9622C36.5566 14.3281 36.0425 13.814 35.4084 13.814C34.7743 13.814 34.2602 14.3281 34.2602 14.9622V16.0773C31.8909 16.7243 30.366 19.0246 30.6923 21.4589C31.0186 23.8933 33.0958 25.7103 35.552 25.7097C36.9726 25.7097 38.1315 26.8473 38.1573 28.2674C38.1836 29.6875 37.0668 30.8671 35.6473 30.9187Z"
    fill="#1D1D1B"/>
<path
    d="M18.9023 24.6501C18.9023 33.8458 26.3567 41.3001 35.5524 41.3001C44.7481 41.3001 52.2024 33.8458 52.2024 24.6501C52.2024 15.4543 44.7481 8 35.5524 8C26.3612 8.01009 18.913 15.4588 18.9023 24.6501ZM35.5524 10.2964C43.4799 10.2964 49.906 16.7225 49.906 24.6501C49.906 32.5776 43.4799 39.0037 35.5524 39.0037C27.6249 39.0037 21.1987 32.5776 21.1987 24.6501C21.2077 16.7264 27.6288 10.3054 35.5524 10.2964Z"
    fill="#1D1D1B"/>
<path
    d="M57.8633 44.1309L48.8689 48.3806C47.6271 46.0707 45.2449 44.6007 42.6233 44.5267L34.3202 44.2985C33.0054 44.261 31.7154 43.9279 30.5464 43.3241L29.7016 42.8863C25.3756 40.6314 20.2194 40.637 15.8979 42.9008L15.9506 40.9868C15.968 40.3527 15.4684 39.8246 14.8344 39.8072L5.72947 39.5566C5.09538 39.5392 4.56781 40.0387 4.55043 40.6728L4.00044 60.6531C3.98306 61.2872 4.4826 61.8153 5.11669 61.8327L14.2216 62.0833H14.253C14.8753 62.0833 15.3843 61.5883 15.4012 60.9665L15.4275 60.0106L17.7929 58.7441C18.7213 58.2446 19.8067 58.1257 20.8209 58.4128L34.9458 62.3771C34.9705 62.3844 34.9952 62.39 35.0204 62.3956C36.0413 62.6092 37.0819 62.7163 38.1247 62.7157C40.3336 62.7169 42.5168 62.2409 44.5245 61.3192C44.5738 61.2968 44.6214 61.2704 44.6669 61.2407L65.1467 47.9937C65.6569 47.6635 65.8217 46.9936 65.5229 46.4643C64.0024 43.7665 60.6301 42.7388 57.8633 44.1309ZM6.32768 59.5688L6.81432 41.8838L13.6234 42.0711L13.1367 59.7567L6.32768 59.5688ZM43.4901 59.2683C40.9958 60.3941 38.2099 60.7047 35.5289 60.157L21.4421 56.2027C19.8566 55.7548 18.1595 55.9403 16.7092 56.7202L15.5004 57.3677L15.8239 45.6093C19.6542 43.0617 24.5666 42.7999 28.6459 44.9253L29.4907 45.3632C30.9675 46.1257 32.5962 46.5462 34.2574 46.5944L42.5611 46.8226C44.8934 46.8898 46.8741 48.5505 47.3473 50.8357L34.9789 50.4942C34.3448 50.4768 33.8173 50.9769 33.7993 51.6105C33.7819 52.2446 34.282 52.7727 34.9156 52.7901L48.565 53.1657H48.597C49.2187 53.1651 49.7272 52.6701 49.7441 52.0489C49.7581 51.5499 49.7211 51.0504 49.6331 50.5593L58.8585 46.2002C58.8675 46.1963 58.8764 46.1918 58.8854 46.1873C60.1939 45.5252 61.7761 45.7467 62.8525 46.7429L43.4901 59.2683Z"
    fill="#1D1D1B"/>
</svg>

                            </span>
                            <Card.Title className="advantages__title mt-4">Удобные способы оплаты</Card.Title>
                            <Card.Subtitle className="advantages__subtitle text-muted mt-2">Мы предлагаем
                                возможность безналичной оплаты</Card.Subtitle>

                        </Card.Body>
                    </Card>
                    <Card className="main__card advantages__card py-3">
                        <Card.Body style={{padding: "none"}}>
                            <span className="advantages__svg ">
                                <svg width="70" height="70" viewBox="0 0 70 70" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
<path
    d="M35.6473 30.9187C35.5178 30.8907 35.3849 30.8856 35.2537 30.9052C33.939 30.7522 32.9472 29.6399 32.945 28.3167C32.945 27.6827 32.4308 27.1685 31.7968 27.1685C31.1627 27.1685 30.6486 27.6827 30.6486 28.3167C30.6519 30.5262 32.1298 32.4616 34.2602 33.0463V34.1946C34.2602 34.8286 34.7743 35.3428 35.4084 35.3428C36.0425 35.3428 36.5566 34.8286 36.5566 34.1946V33.1159C39.0156 32.6012 40.6869 30.311 40.4285 27.8122C40.1694 25.3134 38.0642 23.4145 35.552 23.4139C34.1122 23.4139 32.945 22.2466 32.945 20.8069C32.945 19.3672 34.1122 18.1999 35.552 18.1999C36.9917 18.1999 38.159 19.3672 38.159 20.8069C38.159 21.441 38.6731 21.9551 39.3072 21.9551C39.9413 21.9551 40.4554 21.441 40.4554 20.8069C40.452 18.4869 38.8267 16.486 36.5566 16.0072V14.9622C36.5566 14.3281 36.0425 13.814 35.4084 13.814C34.7743 13.814 34.2602 14.3281 34.2602 14.9622V16.0773C31.8909 16.7243 30.366 19.0246 30.6923 21.4589C31.0186 23.8933 33.0958 25.7103 35.552 25.7097C36.9726 25.7097 38.1315 26.8473 38.1573 28.2674C38.1836 29.6875 37.0668 30.8671 35.6473 30.9187Z"
    fill="#1D1D1B"/>
<path
    d="M18.9023 24.6501C18.9023 33.8458 26.3567 41.3001 35.5524 41.3001C44.7481 41.3001 52.2024 33.8458 52.2024 24.6501C52.2024 15.4543 44.7481 8 35.5524 8C26.3612 8.01009 18.913 15.4588 18.9023 24.6501ZM35.5524 10.2964C43.4799 10.2964 49.906 16.7225 49.906 24.6501C49.906 32.5776 43.4799 39.0037 35.5524 39.0037C27.6249 39.0037 21.1987 32.5776 21.1987 24.6501C21.2077 16.7264 27.6288 10.3054 35.5524 10.2964Z"
    fill="#1D1D1B"/>
<path
    d="M57.8633 44.1309L48.8689 48.3806C47.6271 46.0707 45.2449 44.6007 42.6233 44.5267L34.3202 44.2985C33.0054 44.261 31.7154 43.9279 30.5464 43.3241L29.7016 42.8863C25.3756 40.6314 20.2194 40.637 15.8979 42.9008L15.9506 40.9868C15.968 40.3527 15.4684 39.8246 14.8344 39.8072L5.72947 39.5566C5.09538 39.5392 4.56781 40.0387 4.55043 40.6728L4.00044 60.6531C3.98306 61.2872 4.4826 61.8153 5.11669 61.8327L14.2216 62.0833H14.253C14.8753 62.0833 15.3843 61.5883 15.4012 60.9665L15.4275 60.0106L17.7929 58.7441C18.7213 58.2446 19.8067 58.1257 20.8209 58.4128L34.9458 62.3771C34.9705 62.3844 34.9952 62.39 35.0204 62.3956C36.0413 62.6092 37.0819 62.7163 38.1247 62.7157C40.3336 62.7169 42.5168 62.2409 44.5245 61.3192C44.5738 61.2968 44.6214 61.2704 44.6669 61.2407L65.1467 47.9937C65.6569 47.6635 65.8217 46.9936 65.5229 46.4643C64.0024 43.7665 60.6301 42.7388 57.8633 44.1309ZM6.32768 59.5688L6.81432 41.8838L13.6234 42.0711L13.1367 59.7567L6.32768 59.5688ZM43.4901 59.2683C40.9958 60.3941 38.2099 60.7047 35.5289 60.157L21.4421 56.2027C19.8566 55.7548 18.1595 55.9403 16.7092 56.7202L15.5004 57.3677L15.8239 45.6093C19.6542 43.0617 24.5666 42.7999 28.6459 44.9253L29.4907 45.3632C30.9675 46.1257 32.5962 46.5462 34.2574 46.5944L42.5611 46.8226C44.8934 46.8898 46.8741 48.5505 47.3473 50.8357L34.9789 50.4942C34.3448 50.4768 33.8173 50.9769 33.7993 51.6105C33.7819 52.2446 34.282 52.7727 34.9156 52.7901L48.565 53.1657H48.597C49.2187 53.1651 49.7272 52.6701 49.7441 52.0489C49.7581 51.5499 49.7211 51.0504 49.6331 50.5593L58.8585 46.2002C58.8675 46.1963 58.8764 46.1918 58.8854 46.1873C60.1939 45.5252 61.7761 45.7467 62.8525 46.7429L43.4901 59.2683Z"
    fill="#1D1D1B"/>
</svg>

                            </span>
                            <Card.Title className="advantages__title mt-4">Удобные способы оплаты</Card.Title>
                            <Card.Subtitle className="advantages__subtitle text-muted mt-2">Мы предлагаем
                                возможность безналичной оплаты</Card.Subtitle>

                        </Card.Body>
                    </Card>

                </div>
            </section>


        </div>
    );
};

export default Main;



